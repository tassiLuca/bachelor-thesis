...
In seguito, la sequenza di \textit{token} generata viene eventualmente filtrata e successivamente confrontata con algoritmi di \textit{string matching}. 

La fase di filtraggio consiste nel ridurre il numero di rappresentazioni da dover confrontare e, quindi, il numero stesso dei confronti, portando a una riduzione dei tempi di esecuzione.
%
Questo stadio è preceduto da una fase d'indicizzazione, in cui vengono generati alcuni indici a partire dalla sequenza di \textit{token}.
%
Una tra gli indici più comuni è costituito da coppie chiave$-$valore in cui nelle chiavi sono memorizzati i tipi di \textit{token} e nei valori la loro frequenza.
%
Un esempio di indice a costruito a partire dalla sequenza di \textit{token} elencati in \Cref{table:token-associations} è mostrato in \Cref{table:token-indexing}.
...
\begin{table}[h!]
    ...
    \end{tabular}
    \caption{Indice con coppie tipo di \textit{token}$-$frequenze generate a partire dalla \Cref{table:token-associations}}
    \label{table:token-indexing}
\end{table}

Tali indici sono successivamente impiegati nella vera e propria fase di filtraggio, in cui viene applicata una particolare metrica, ad esempio la similarità coseno, e viene utilizzato un valore di soglia per escludere coppie di sorgenti che hanno più bassa similarità in accordo con la metrica scelta.

\todo{...}

A seguire i sorgenti vengono confrontati.
%
I due algoritmi di confronto più conosciuti sono \textbf{\textit{Winnowing}} \cite{winnowing} e \textbf{\textit{Running-Karp-Rabin Greedy String Tiling (RKR-GST)}} \cite{wise-running-93}.
...
%
I dettagli sono descritti nella \Cref{03-matching-algorithms}.
...
In figura \Cref{img:01-ast} viene mostrata la struttura parziale dell'\textit{AST} del metodo \texttt{main()} del \Cref{lst:test-analysis}.
...
\lstinputlisting[
    float,
    language=c,
    caption={Somma di un array, in C},
    label={lst:pdg}
