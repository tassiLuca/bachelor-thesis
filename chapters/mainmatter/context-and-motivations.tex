...
% vedi Overwiew and Comparison Of Plagiarism... 1 Paragrafo - Plagiarism can be classified into five ...
...
Con lo sviluppo della tecnologia, molti ricercatori hanno fatto progressi in questo campo e proposto diversi algoritmi negli ultimi anni, formando gradualmente tre popolari famiglie di tecniche: \textbf{\textit{attribute-based}}, \textbf{\textit{structure-based}}, \textbf{\textit{hybrid detection}}. \todo{citazione?}

\subsection{Analisi \textit{attribute-based}}
Il primo sistema automatico di rilevazione di codice copiato che sia stato documentato in letteratura risale al 1976 (\cite{ottenstein}) e utilizzava le seguenti quattro metriche di Healstead per determinare il livello di similarità tra coppie di sorgenti \cite{halstead}:
...
I successivi sistemi hanno sperimentato queste e altre metriche, come il numero \textit{loop} e il numero di procedure, per trovare misure più accurate di somiglianza e che riflettano le strutture di controllo del flusso.

Collettivamente, questi sistemi sono stati chiamati sistemi \textit{attribute-counting-metric}.
%
Alcuni tipi di tali misurazioni sono adattati da altri domini.
%
Ad esempio, due principali tecniche di determinazione della similarità in questo ambito sono la \textit{Latent Semantic Analysis} e tecniche di \textit{Clustering}.

\todo{LSA}

\todo{Clustering Approach}

Considerando che le caratteristiche del codice sorgente non sono strettamente correlate alla semantica del programma e che la misurazione della somiglianza può risultare essere imprecisa, l'efficacia di queste tecniche è considerevolmente limitata.
%
Inoltre, l'efficacia di tali tecniche è inferiore, in termini di precisione nell'identificazione di plagi, rispetto alle tecniche \textit{structure-based} \cite{es-plag}.
...
%
% immagine struttura compilatore?
%
...
%
Tale albero è definito "astratto" in quanto non rappresenta ogni dettaglio tipico della sintassi del linguaggio, bensì solo i dettagli strutturali e relativi al contenuto.
%
