...
\begin{equation}
	\begin{split}
		sim_P(A, B) &= max \biggl\{ \frac{|\textit{sorgenti segnalati di A}|}{|\textit{sorgenti di A}|}, \frac{|\textit{sorgenti segnalati di B}|}{|\textit{sorgenti di B}|} \biggr\} \\
		&  \cdot P_{75}(\textit{similarità sorgenti segnalati})
	\end{split}	
\end{equation}
...
%
Per questo motivo, la misura di similarità aggregata sopra descritta viene "pesata" sulla base del massimo rapporto tra il numero di sorgenti segnalati di $A$ e $B$ e quelli del progetto in esame.
%
...
%
Questo tuttavia, ha un effetto negativo: consideriamo ad esempio che \dots
