...
Nella versione originale dell'algoritmo la struttura dati utilizzata per memorizzare i \textit{match} è una lista di code in cui ogni coda memorizza i \textit{match} della stessa lunghezza, mantenuta ordinata per valori descrenti di lunghezza.
...
%
La principale differenza con la precedente versione risiede nel fatto che la marcatura dei \textit{match} viene effettuata sulla base...
...
%
Come si può osservare, corrispondenze molto lunghe vengono trovate solo durante la prima iterazione e dopo di questa non saranno trovate corrispondenze maggiori del doppio della lunghezza di ricerca corrente.
%
Il valore di $s$ (l'equivalente di \texttt{minimum\_match\_length} nell'algoritmo \textit{Greedy String Tiling}) è scelto in modo tale da garantire che non vengano cercate corrispondenze troppo piccole.
%
...
Utilizzando le formule sopra esposte, tuttavia, si va incontro a un difetto intrinseco al tipo di tecnica utilizzata: se dall'applicazione del rilevamento risulta un insieme elevato di sottosequenze \dots

Per mitigare questo problema in \cite{es-plag} viene proposto di modificare la stima formula di similarità introducendo un meccanismo di penalità: maggiore sono il numero di sottosequenze che fanno \textit{match}, maggiore sarà la penalità applicata...
